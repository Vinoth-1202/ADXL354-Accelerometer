...
void ADXL345_Initialize()
{
    ...
    printf ("%d \n",val);
    val =0x15;//22x62.5mg=1.4375g
    ADXL345_REG_WRITER(THRESH_ACT, &(val), 1 );
    printf ("%d \n",val);
    val=0x13;//21x62.5mg=1.3125g
...
void def()
{
        ...
        printf ("gpio pins are configured and isr is installed\n");
}
    ...
    while(1)
    {
       ...
       /*int i,j;
        ...
        if (activity_flag) {
            printf("Activity detected!\n");
            printf ("%d\n",activity_flag);
            activity_flag = 0; // Reset the flag
        }
        if(inactivity_flag )
        {
            printf("Inactivity detected!\n");
            printf ("%d\n",inactivity_flag);
            inactivity_flag = 0; // Reset the flag
        }
     // printf("loop is going on \n");
 //  printf ("%d \n",activity_flag);
   // printf ("%d \n",inactivity_flag);
    vTaskDelay(500);
    }
